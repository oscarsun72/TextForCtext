...
'Const description As String = "將星號前的分段符號移置前段之末 & 清除頁前的分段符號{佛弟子文獻學博士孫守真任真甫按：仁者志士義民菩薩賢友請多利用賢超法師《古籍酷AI》或《看典古籍》OCR事半功倍也。二站每日均有不少的免費額度可供善用，願毋荒蕪、空拋虛擲了！如蒙不棄，可利用末學自製於GitHub開源免費免安裝之TextForCtext 應用程式，加速輸入與排版。討論區與末學YouTube頻道有演示影片可資參考。感恩感恩　讚歎讚歎　南無阿彌陀佛　讚美主}"
...
Const description As String = "將星號前的分段符號移置前段之末 & 清除頁前的分段符號{佛弟子文獻學博士孫守真任真甫按：據北京元引科技有限公司《元引科技引得數字人文資源平臺·中國歷代文獻》所藏本輔以末學自製於GitHub開源免費免安裝之TextForCtext排版對應錄入。討論區與末學YouTube頻道有實境演示影片可資參考。感恩感恩　讚歎讚歎　南無阿彌陀佛　讚美主}"
...
Sub formatter年前加分段符號() '為《經典釋文》春秋三傳等格式用，日後可改成其他需要格式化的文本
    ...
    d.Close wdDoNotSaveChanges
End Sub
Sub 維基文庫四部叢刊本轉來(Optional lineCntPerPage As Byte = 0)
    ...
    If pp Then
        If IsNumeric(Mid(xClip, pp + 1, 1)) Then
            元引科技引得數字人文資源平臺_北京元引科技有限公司轉來 lineCntPerPage
            Exit Sub
        End If
    End If
    ...
    '提示貼上無礙
    SystemSetup.playSound 1
    ClipBoard.SetClipboard vbNullString
...
End Sub
Rem 現在多用Kanripo.org者 20250202大年初五；lineCntPerPage 每頁行數，以供檢查補空行
Sub 元引科技引得數字人文資源平臺_北京元引科技有限公司轉來(Optional lineCntPerPage As Byte = 0)
...
'            rng.Find.Execute "[ ]", , , , , , , wdFindContinue, , vbNullString, wdReplaceAll
    ...
    If lineCntPerPage > 0 Then
        ...
        Do While rng.Find.Execute(vbNullString, , , , , , True, wdFindContinue)
            ...
            If InStr(rng.Document.Range(rngExamLineCntPerPage.start, rng.Document.Range.End), "P") = 0 Then '後面沒有「P」了，處理完了，就離開
...
'                    If line.LineRange(noteRng).start = noteRng.start And line.LineRange(noteRng).End = noteRng.End Then
                    ...
                    ElseIf noteRng.Next = VBA.Chr(13) Then 'ax=縮排的空格 ●●●●●●●●●●●●●
                        insertX = VBA.Chr(13) & aX  'VBA.Chr(13) 後面 a.text = "}}" & VBA.Replace(insertX, VBA.Chr(13), VBA.Chr(13) & "{{") 要參照
                    Else
                        If VBA.InStr(midNoteRng.text, "/") _
                            And noteRng.Next.font.Size > 11.5 _
                            And (noteRng.Next.text <> VBA.Chr(13) Or noteRng.Next.text = "　") Then  '若是夾注(通常是標題下的夾注（則後面有空格），如 https://ctext.org/library.pl?if=en&file=55677&page=6） 20250205
                            'noteRng.Next.text <> VBA.Chr(13):後面還有文字，則為夾注 20250223補
                            insertX = aX '補空格以縮排
                        Else
...
'        .Range.text = Replace(Replace(.Range.text, VBA.Chr(11), Chr(13) & Chr(10)), "?", "/")'●●●●●●●●●●●●●●●●●●●
        .Range.text = VBA.Replace(.Range.text, "?", "/") '將造字亂碼改為「/」以供後續程式之識別 20260101元旦
        .Range.Cut
...
'    DoEvents
End Sub
Sub Kanripo_GitHub轉來(Optional lineCntPerPage As Byte = 0)
    'https://github.com/kanripo
    '以下問題已解決，待觀察
    '還未完善，如以下此檔之獨立注文處：
    'https://github.com/kanripo/KR4h0160/blob/master/KR4h0160_046.txt
    Dim rng As Range, d As Document, p As Paragraph, noteRng As Range, i As Long, a As Range, exa_rng As Range, xd As String, si As New StringInfo ', ac_noteRng As Long,
    ...
    Dim lt As String '目前只用得到「<」, gt As String '如「&lt;pb:KR2k0003_WYG_015-1a&gt;」這樣的情形 https://copilot.microsoft.com/shares/VhSHyY1MbFaYKJ6MDH3hE
    Dim amp As String
    If InStr(rng.Document.Range, "&lt;") Then '在TextForCtext C#程式碼裡新加入了 SelectAndCopyElementHtmlContent 這樣讀取html時「<、>」會成了如上如左「&lt;、&gt;」的樣子，不會是「<」與「>」
        lt = "&lt;"  ': gt = "&gt;"
        amp = "&" '"&amp;"'因為「查字.mdb」資料庫裡存的是「&KR1335;」這樣的值，故先轉 "&amp"為"&"
        d.Range.text = VBA.Replace(d.Range.text, "&amp;", "&")
    Else
        lt = Network.HtmlEntity("&lt;") '"<" ': gt = ">"
        amp = Network.HtmlEntity("&amp;") '"&"
    End If
...
'    End If
    ...
    Rem 清除標頭
    Do While d.Paragraphs(i + 1).Range.Characters(1).text Like "[#" & Chr(13) & "]"
...
'    End If
    ...
    'https://darkranger.no-ip.org/terminology/Placeholder Placeholder（佔位字元、佔位符）
    '與《元引科技引得數字人文資源平臺·中國歷代文獻》本均係純文字字無圖（圖在word文件中的佔位字元即「/」，蓋缺字多用字圖表示故也），以「/」表示小注換行之處，故先取代為\以與造字缺字者區別 20260104
    rng.Find.Execute findText:="/", Replacewith:="\", Replace:=wdReplaceAll, Wrap:=wdFindContinue
    
    
    Rem 取代KanripoGitHub存放庫的造字圖碼
    If VBA.InStr(d.Range.text, amp & "KR") Then
        Dim db As New dBase, cnt As ADODB.Connection, rst As New ADODB.Recordset
        db.cnt查字 cnt
        rst.Open "KanripoGitHub存放庫造字圖碼取代對照表", cnt, adOpenStatic
        xd = d.Range.text
        Do Until rst.EOF
            If VBA.InStr(xd, rst.Fields(0).Value) Then
                playSound 0.411
'                Debug.Print rst.Fields(0) & VBA.vbCr
                'd.Range.Find.Execute findText:=rst.Fields(0).Value, Wrap:=wdFindContinue, Replacewith:=rst.Fields(1).Value, Replace:=wdReplaceAll
                '因為都是純文字處理的，所以用這個比較快
                xd = VBA.Replace(xd, rst.Fields(0).Value, rst.Fields(1).Value)
            End If
            rst.MoveNext
        Loop
        d.Range.text = xd
        rst.Close
        'cnt.Close
        'Set db = Nothing '後面還有用
    End If

    Rem 先取代造字編碼（如：&KR1016;）為其本字或「/」才好作字數計算 20260104
    Set rng = d.Range: xd = rng.text
    Do While rng.Find.Execute(findText:=amp & "KR", Wrap:=wdFindStop, Forward:=True)
        Set noteRng = rng.Duplicate
        noteRng.MoveEndUntil ";"
        noteRng.MoveEnd 1
        If amp <> "&" Then
...
'        'rng.Find.ClearFormatting
    ...
    Do While rng.Find.Execute("(", Wrap:=wdFindStop, Forward:=True)
...
'        End If
        ...
        'If VBA.InStr(noteRng.text, "/") Then
        If VBA.InStr(noteRng.text, "\") Then
            'ac_noteRng = noteRng.Characters.Count '這是是否為夾注/小注分行處之判斷，現在已改為「\」，故不需要再與造字缺字之「/」作識別了。一律都要處理
            For Each a In noteRng.Characters
                i = i + 1
                If a = "\" Then
                    'If i = Int(ac_noteRng / 2) + 1 Then '這是是否為夾注/小注分行處之判斷，現在已改為「\」，故不需要再與造字缺字之「/」作識別了。一律都要處理
...
'                                End If
                            ...
                            Else
                                ...
                                Else
                                    a.InsertAfter "　" '注文只有1字而後接著「/」時仍補全形空格1個
                                End If
                                'a.Delete '清除/
...
'                            Stop
                        ...
                        End If
'                    Else'If i = Int(ac_noteRng / 2) + 1 Then '這是是否為夾注/小注分行處之判斷，現在已改為「\」，故不需要再與造字缺字之「/」作識別了。一律都要處理
'                        If a.Next = ")" Then
'                            'If noteRng.Characters.Count > 1 Then
'                            If i > 2 Then
'                                Set p = rng.Paragraphs(1)
...
'                    End If
        ...
        End If
    Loop
    '將縮排易傳獨立注文其標記位置與全形空格對調，以便在ctext.org中排版
    For Each p In d.Range.Paragraphs
        If p.Range.Characters(1).text = "　" Then
            If InStr(p.Range.text, "(") Then
                rng.SetRange p.Range.start, p.Range.start
                rng.MoveEndUntil "("
                If Replace(rng.text, "　", vbNullString) = vbNullString Then
'                    rng.Select 'just for test
    ...
    End If
    Next p
    d.Range.Find.Execute findText:="(", Replacewith:="{{", Replace:=wdReplaceAll
    d.Range.Find.Execute findText:=")", Replacewith:="}}", Replace:=wdReplaceAll
    If InStr(d.Range.text, ChrW(63)) Then
        d.Range.text = VBA.Replace(d.Range.text, ChrW(63), "/") '取代「?」(應是缺字）為「/」
    End If
'    Do While d.Range.Find.Execute(findText:="^p　^p", Wrap:=wdFindStop)
'        d.Range.Find.Execute findText:="^p　^p", replacewith:="^p|^p", Replace:=wdReplaceAll
'    Loop
...
Sub 國學大師_Kanripo_四庫全書本轉來(Optional lineCntPerPage As Byte = 0)
...
'    End If
    ...
    '        .Replacement.Text = vba.Chrw(12295)
            .Replacement.text = "　}}"
            .Execute , , , , , , True, wdFindContinue, , , wdReplaceAll
            '.Execute Chr(11), , , , , , True, wdFindContinue, , Chr(13), wdReplaceAll
        End With
        '.Range.Cut
...
restart:
    ...
    Else '連續輸入時，以Normal.dotm範本以外的範本（如 TextForCtext-WordVBA.dotm）開啟之新文件者
        ...
        ElseIf preFileNum > 0 And VBA.Left(d.Range, 1) = "-" And VBA.IsNumeric(VBA.Trim(d.Range.text)) Then '如果文本只是以「-」開頭，且是數字的話 20260214
            d.Range.text = d.Range.text & Chr(13) & Chr(13) & Chr(13) & GetClipboardText()
...
Sub 新頁面Auto_action_newchapter()
    ...
    title = VBA.Replace(title, head, vbNullString)
    If VBA.Left(title, 1) = "·" Then title = Mid(title, 2)
...
Sub Quickedit_Syntax_Convert2_Edit()
    ...
    Set d = Documents.Add
    fileNo = InputBox("請輸入file number", , fileNo)
    If VBA.IsNumeric(fileNo) = False Then Exit Sub
    d.Range.Paste: Set rng = d.Range
    xd = d.Range.text
    For i = 100 To 2 Step -1
        sp = StrConv(space(i), vbWide)
        If InStr(xd, sp) Then
            Do While rng.Find.Execute(findText:=sp, Wrap:=wdFindStop)
                If rng.Characters(1).Previous = Chr(13) Then
                    xReplace = "<scanbreak file=""" & fileNo & """ y=""" & i & """ />"
                Else
                    xReplace = "<scanskip file=""" & fileNo & """ y=""" & CStr(i) & """ />"
                End If
                d.Range.Find.Execute findText:=sp, Replacewith:=xReplace, Replace:=wdReplaceAll, Wrap:=wdFindContinue
            Loop
            Set rng = d.Range
        End If
    Next i
    Set rng = d.Range: sp = "　"
    If d.Range.Characters(1) = "　" Then d.Range.InsertParagraphBefore
    Do While rng.Find.Execute(findText:="[" & sp & "]", Wrap:=wdFindContinue, MatchWildcards:=True)
        If rng.Characters(1).Previous = Chr(13) Then
            xReplace = "<scanbreak file=""" & fileNo & """ y=""" & i & """ />"
            rng.Characters(1).Previous.Delete
        Else
            xReplace = "<scanskip file=""" & fileNo & """ y=""" & CStr(i) & """ />"
        End If
        rng.text = xReplace
    ...
    Loop
    a(0) = Chr(13):                                    a(1) = "<p>":                a(2) = ChrW(-9217) & ChrW(-8195)
    b(0) = "<scanbreak file=""" & fileNo & """ />":    b(1) = Chr(13) & Chr(13):    b(2) = "　"
    For i = 0 To UBound(a)
        d.Range.Find.Execute findText:=a(i), Replacewith:=b(i), Replace:=wdReplaceAll, Wrap:=wdFindContinue
    Next i
    
    d.Range.text = Replace(Replace(d.Range.text, ChrW(8220), ChrW(34)), ChrW(8221), ChrW(34))
    d.Range.Copy
    playSound 1
