...
'    If VBA.InStr(rng.text, "¡D¡@¡D¡@¡D¡@¡D¡@¡D¡@¡D¡@¡D¡@¡D¡@¡D¡@¡D¡@¡D¡@¡D¡@¡D¡@¡D¡@¡D¡@¡D¡@¡D¡@¡D") Then Stop 'just for test
    ...
    If yi Then
        ...
        Else Rem °e¥h¡m¥jÄy»Å¡n¦Û°Ê¼ÐÂI
            If Docs.mark©ö¾ÇÃöÁä¦r(Nothing, True) Then
...
'                Stop 'just for test
                ...
                ¤å¦r³B²z.FixFontname pasteAppendedRange
                SystemSetup.playSound 2
                Rem ¥H¤U¬°ÂÂ¦¡
...
'                End If
                ...
                For Each p In pasteAppendedRange.Paragraphs
                    If VBA.Len(p.Range.text) < 10 And VBA.Len(p.Range.text) > 1 Then
                        If VBA.IsNumeric(VBA.Replace(VBA.Replace(VBA.Replace(p.Range.text, Chr(13), vbNullString), "-", vbNullString), "¡C", vbNullString)) Then
                            Set pManageRng = p.Range.Document.Range(p.Range.start, p.Range.End - 1)
                            pManageRng.text = VBA.Replace(pManageRng.text, "¡C", vbNullString)
...
End Function
Sub ¹y¸¹¤å¦r¥[¤W½g¦W¸¹¦p©P©ö½Ñ¨ö()
    Rem Ctrl + Alt + ' ¡]¹y¸¹¡^
    ¹y¸¹¤å¦r¥[¤W²Å¸¹ "¡q"
End Sub
Sub ¹y¸¹¤å¦r¥[¤W®Ñ¦W¸¹¦p¤­¸g()
    Rem Ctrl + shift + Alt + ' ¡]¹y¸¹¡^
    ¹y¸¹¤å¦r¥[¤W²Å¸¹ "¡m"
End Sub
Sub ¹y¸¹¤å¦r¥[¤W³æ¤Þ¸¹()
    Rem Ctrl + Alt + 9
...
Rem ¦¨¥\«h¶Ç¦^true
Private Function ¹y¸¹¤å¦r¥[¤W²Å¸¹(symbol As String) As Boolean
    ...
    Select Case symbol
        ...
        Case Else
            Exit Function
    End Select
    SystemSetup.stopUndo ur, "¹y¸¹¤å¦r¥[¤W½g¦W¸¹¦p©P©ö½Ñ¨ö"
    word.Application.ScreenUpdating = False
    ¤å¦r³B²z.ResetSelectionAvoidSymbols
...
'        x = symbol & VBA.Replace(rng.text, "¡B", symbolClose & "¡B" & symbol) & symbolClose
...
'    Else
        ...
        Else
            If rng.Characters(1).text <> symbol And rng.Characters(1).text <> "¡B" Then rng.InsertBefore symbol
        End If
...
Sub FixFontnames()
    ...
    word.Application.ScreenUpdating = True
End Sub
Rem ®Ú¾Ú½X¦ì¨Ó³]©w¦r«¬¦WºÙ¡C¥¢±Ñ«h¶Ç¦^false
Function FixFontname(rng As Range) As Boolean
    ...
    '    Docs.ChangeFontOfSurrogatePairs_Range "HanaMinA", d.Range(selection.Paragraphs(1).Range.start, d.Range.End), CJK_Compatibility_Ideographs
        ...
        If Not Fonts.IsFontInstalled("HanaMinB") Then
            If Not Fonts.IsFontInstalled("¥þ§ºÅé(µ¥¼e)") Then
                MsgBox "½Ð¥ý¦w¸Ë""HanaMinB""¦r«¬¦A¸Õ¤@¦¸¡I", vbExclamation
        ...
        If Not Fonts.IsFontInstalled("¥þ§ºÅé-3") Then
            MsgBox "½Ð¥ý¦w¸Ë""¥þ§ºÅé""¦r«¬¦A¸Õ¤@¦¸¡I", vbExclamation
...
Sub ¤j³°¤Þ¸¹§ï»OÆW¤Þ¸¹()
    Rem Alt + l ¡]¦³¿ï¨ú¥u°õ¦æ¿ï¨ú°Ï¡AµL«h¾ã¥÷¤å¥ó¡^
    Rem ¤µ§ï¦¨·PÀ³¦¡, §Y­Y°»´ú¿ï¨ú°Ï¦³¤j³°¤Þ¸¹, «h°õ¦æ³°Âà»O, §_«h¬°»OÂà³° 20241230
    ...
    Else
        ...
        clnRng.Add rng
    End If
    SystemSetup.stopUndo ur, "»OÆW¤j³°¤Þ¸¹Âà´«" ',"¤j³°¤Þ¸¹§ï»OÆW¤Þ¸¹"
    word.Application.ScreenUpdating = False
    
    Dim smartQuotes As Boolean ' «O¦s·í«e³]¸m
    smartQuotes = options.AutoFormatAsYouTypeReplaceQuotes ' ¼È®ÉÃö³¬´¼¯à¤Þ¸¹¦Û°Ê®Õ¥¿¥\¯à
    options.AutoFormatAsYouTypeReplaceQuotes = False
        
    For Each rngStory In clnRng
        ' °õ¦æ§A»Ý­nªº¾Þ§@ ' <§Aªºµ{¦¡½X>
        Set rng = rngStory
        If VBA.InStr(rng.text, VBA.ChrW(8220)) Or VBA.InStr(rng.text, VBA.ChrW(8221)) Or VBA.InStr(rng.text, VBA.ChrW(8216)) Or VBA.InStr(rng.text, VBA.ChrW(8217)) Then
            If VBA.InStr(rng.text, VBA.ChrW(8220)) Then
                rng.Find.Execute VBA.ChrW(8220), , , , , , , wdFindStop, , "¡u", wdReplaceAll
            End If
            If VBA.InStr(rng.text, VBA.ChrW(8221)) Then
                rng.Find.Execute VBA.ChrW(8221), , , , , , , wdFindStop, , "¡v", wdReplaceAll
            End If
            If VBA.InStr(rng.text, VBA.ChrW(8216)) Then
                rng.Find.Execute VBA.ChrW(8216), , , , , , , wdFindStop, , "¡y", wdReplaceAll
            End If
            If VBA.InStr(rng.text, VBA.ChrW(8217)) Then
                rng.Find.Execute VBA.ChrW(8217), , , , , , , wdFindStop, , "¡z", wdReplaceAll
            End If
        Else
            If VBA.InStr(rng.text, "¡u") Then
                's = rng.start: e = rng.End
                rng.Find.Execute "¡u", , , , , , , wdFindStop, , VBA.ChrW(8220), wdReplaceAll
                Rem ¦³ options ªº³]©w´N¤£¥²¶]°j°é¤F 20241230
        '        Do While rng.Find.Execute("¡u")
        '            rng.text = VBA.ChrW(8220)
        '            rng.SetRange s, e
        '        Loop
            End If
            If VBA.InStr(rng.text, "¡v") Then
                rng.Find.Execute "¡v", , , , , , , wdFindStop, , VBA.ChrW(8221), wdReplaceAll
            End If
            If VBA.InStr(rng.text, "¡y") Then
                rng.Find.Execute "¡y", , , , , , , wdFindStop, , VBA.ChrW(8216), wdReplaceAll
        '        Do While rng.Find.Execute("¡y")
        '            rng.text = VBA.ChrW(8216)
        '            rng.SetRange s, e
        '        Loop
            End If
            If VBA.InStr(rng.text, "¡z") Then
                rng.Find.Execute "¡z", , , , , , , wdFindStop, , VBA.ChrW(8217), wdReplaceAll
            End If
        End If
        
        Set rng = rngStory
        rng.Find.Execute findText:=",", Replacewith:="¡A", Replace:=wdReplaceAll
        rng.Find.Execute findText:=";", Replacewith:="¡F", Replace:=wdReplaceAll
        rng.Find.Execute findText:=":", Replacewith:="¡G", Replace:=wdReplaceAll
        rng.Find.Execute findText:="?", Replacewith:="¡H", Replace:=wdReplaceAll
            ...
            Do While aPre.text <> VBA.Chr(13)
                Select Case aPre.text
                    Case "¡u"
                        Set rng = rngStory
                        With rng.Find
                            .Execute "¡y", , , , , , , wdFindStop, , VBA.Chr(1), wdReplaceAll
                            .Execute "¡u", , , , , , , wdFindStop, , "¡y", wdReplaceAll
                            .Execute VBA.Chr(1), , , , , , , wdFindStop, , "¡u", wdReplaceAll
                            
                            .Execute "¡z", , , , , , , wdFindStop, , VBA.Chr(1), wdReplaceAll
                            .Execute "¡v", , , , , , , wdFindStop, , "¡z", wdReplaceAll
                            .Execute VBA.Chr(1), , , , , , , wdFindStop, , "¡v", wdReplaceAll
                    ...
                    Case VBA.ChrW(8220)
                        ...
                        Set rng = rngStory
                        With rng.Find
                            'VBA.ChrW(8216)¤j³°ªº¤W³æ¤Þ¸¹ VBA.ChrW(8217)¤j³°ªº¤U³æ¤Þ¸¹
                            'VBA.ChrW(8220)¤j³°ªº¤WÂù¤Þ¸¹ VBA.ChrW(8221)¤j³°ªº¤UÂù¤Þ¸¹
...
Sub ºû°ò¤å®w¥hª`¤å()
    Dim rng As Range, ur As UndoRecord
    SystemSetup.stopUndo ur, "ºû°ò¤å®w¥hª`¤å"
    ...
    SystemSetup.contiUndo ur
End Sub
Rem ¼ÒÀÀWord Characters ¶°¦Xª«¥óªº¦r¦ê¶°¦X https://learn.microsoft.com/en-us/office/vba/api/Word.characters
...
Rem 20250118 ±NWord¤åÀÉ¤¤¿ï©w¤å¥»¤¤ªº±×½u«e«á¤å¦r¶i¦æ­Ë¸m creedit_with_Copilot¤jµÐÂÄ
Function SwapTextAroundSlash(selectionText As String) As String
    ...
    End If
End Function
Rem 20250118 ±N¤å¥»¤¤¤pª`¤å±×½u«e«áÄA­Ëªº¤å¥»§ï¥¿¹L¨Ó
