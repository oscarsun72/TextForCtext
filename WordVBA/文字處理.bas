...
Sub º~Äy¹q¤l¤åÄm¸ê®Æ®w¤å¥»¾ã²z_¥HÂà¶K¨ì¤¤°ê­õ¾Ç®Ñ¹q¤l¤Æ­p¹º(Optional doNotCloseDoc As Boolean, Optional openNewDoc As Boolean = False)
    ...
    Set d = ActiveDocument
    If (d.path <> "" Or d.Content.text <> Chr(13)) And openNewDoc Then
        Set d = Documents.Add()
        'Exit Sub
    End If
    ...
    End If
    º~Äy¹q¤l¤åÄm¸ê®Æ®w¤å¥»¾ã²z_ª`¤å«e«á¥[¬A¸¹_old
...
'    rng.PasteAndFormat wdFormatPlainText
    rng.text = rng.text '¡mº~Äy¥þ¤å¸ê®Æ®w¡n§ïª©«á°Å¶KÃ¯®Ä¯à«Ü®t¡A³y¦¨µ{§Ç·í±¼¡A§ï¼g¦¨³o¦æ«á´N¸Ñ¨M¤F¡I¥i¨£°ÝÃD¥X¦b°Å¶KÃ¯¡A«o¤£¬O§Ú¼gªºµ{¦¡¡C·P®¦·P®¦¡@Æg¼ÛÆg¼Û¡@«nµLªüÀ±ªû¦ò 20240825
...
Sub º~Äy¹q¤l¤åÄm¸ê®Æ®w¤å¥»¾ã²z_ª`¤å«e«á¥[¬A¸¹() '³Ì«á°õ¦æ Docs.mark©ö¾ÇÃöÁä¦r(¦b¡u©ö¾ÇÂøµÛ¤å¥»¡v¸ô®|¤U®É¡^
    ...
    Else
        Set rng = Docs.ªÅ¥Õªº·s¤å¥ó.Range
    End If
    ...
    rng.Find.ClearFormatting
    If InStr(rng.Document.Range.text, "¡^¡]¡C¡^") Then
        With rng.Find
            .text = "¡^¡]¡C¡^"
            .Replacement.text = "¡C¡^"
...
Sub º~Äy¹q¤l¤åÄm¸ê®Æ®w¤å¥»¾ã²z_ª`¤å«e«á¥[¬A¸¹_old()
