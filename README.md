# TextForCtext
為了[《中國哲學書電子化計劃》（Chinese Text Project,簡稱ctext）](https://ctext.org/)輸入用
...
分行分段按鈕：若有按下Ctrl才按此鈕則執行圖文脫鉤 Word VBA ，在本應用程式介面視窗從作業系統得到焦點、成為作用中的視窗時，只要是在按下 Ctrl 或 Shift 的狀態下，亦會由剪貼簿現存的內容來判斷，是否執行同一VBA程序
...
- 若已複製網址在剪貼簿，則滑鼠點擊或在 textBox3 上滑過即會讀入所複製的網址值給 textBox3；在軟件介面縮小至任務列時，滑過軟件圖示，也會啟動此功能
