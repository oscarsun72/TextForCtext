...
button1 「分行分段」或「送出貼上」按鈕: 
 ...
 - 若有按下Ctrl才按此鈕則執行[圖文脫鉤 Word VBA](https://github.com/oscarsun72/TextForCtext/blob/4e140975f3881bae8e7f5acc5899c7d61a4794d3/WordVBA/%E4%B8%AD%E5%9C%8B%E5%93%B2%E5%AD%B8%E6%9B%B8%E9%9B%BB%E5%AD%90%E5%8C%96%E8%A8%88%E5%8A%83.bas#L346) 
 
 >或在本應用程式介面視窗從作業系統得到焦點、成為作用中的視窗時，只要同時按下Ctrl Alt鍵 ，亦會由剪貼簿現存的內容來判斷，是否要執行同一個圖文脫鉤的VBA程序
 
 >>沒按下的話，預設是執行「中國哲學書電子化計劃.清除頁前的分段符號」程序，如果剪貼簿裡的文字本含有完整「編輯」模式下的文本特徵時（詳程式碼內原理，在[中國哲學書電子化計劃.bas](https://github.com/oscarsun72/TextForCtext/blob/4e140975f3881bae8e7f5acc5899c7d61a4794d3/WordVBA/%E4%B8%AD%E5%9C%8B%E5%93%B2%E5%AD%B8%E6%9B%B8%E9%9B%BB%E5%AD%90%E5%8C%96%E8%A8%88%E5%8A%83.bas#L136)模組檔案裡）。
...
- 當 textBox1、textBox3 內容為空白時，滑鼠左鍵點一下即讀進剪貼簿內容。
    > 當所複製之網址（剪貼簿裡的內容）為 ctext 網站的圖文對照頁面網址時，點擊textBox3，則會自行啟動Selenium模式以啟動Chrome瀏覽器供程式操作。（目前末學習慣都是如此啟用本軟件。20260102）
-   > 若在手動輸入模式下，則會據此剪貼簿中的內容在新頁籤中開啟對應的頁面以供操作。

- 當離開 textBox2 或 textBox4 文字方塊時，即自執行在 textBox1 尋找或取代文字功能（增罝透過 button2 的切換，決定是否只在選取區中執行尋找、取代）
    > 當離開textBox4有按下Shift鍵時，則進行章節單位全部取代（即 Alt + e 所執行之功能） 20241116
- 
...
- 預設為最上層顯示，則按下Esc鍵或滑鼠中鍵會隱藏到任務列（系統列）中；滑鼠在其 ico 圖示上滑過即恢復（若在SeleniumNew+手動輸入模式下，瀏覽圖文對照的頁面時，即會開啟該頁簡單修改模式頁面，並將其內容讀入textBox1中。若按住Shift再滿滑過（或未按下時，已切換到OCR輸入模式（ocrTextMode=true） ，則會直接送交賢超法師《古籍酷AI》OCR，若識讀成功，則直接取回文本並加上[查字.mdb](https://www.dropbox.com/scl/fi/vze6vqx4ooj7dwd6met53/.mdb?rlkey=mh1kl48l75di505wedo771naw&dl=0)資料庫已存在的書名篇名號等標點符號。）
...
>若要煞車，請對Chrome瀏覽器或本操作介面按下`CapsLock`鍵（啟動之，其燈亮）
...
>在完整編輯頁面中直接取代文字。請將被取代+取代成之二字前後並置，並將其選取後（或在被取代之文字前放置插入點）再按下此組合鍵以執行直接取代 20240718

Alt + Insert ：將textBox1內容，自動標上書名號、篇名號、及一些基本斷句標點符號。
    
        > 原是：在textBox1清除原內容，貼上剪貼簿中的內容。若在手動輸入模式下會自動標上書名號、篇名號、及一些基本斷句標點符號。 20241206 今改作。因為之前倚靠剪貼簿來操作，現在不了。

F2 ：全選/取消全選textbox1內容，並將此內容複製到剪貼簿。

數字鍵盤 `+` 或 F8 或 F9 或 F12：在手動輸入模式時，將整頁（textBox1的全內容）送出至[簡單修改模式]（Quick edit）下的文字框中，完成後翻到下一頁，並將焦點拉回表單中。
>若在圓滿前按下 Shift，則直接將下一頁送出指定的OCR網站作處理。
>>所指定的是何網站，詳textBox2中所下達的`df` `kd` `kapi` 等指令。

Ctrl + 數字鍵盤 `-` ：重新指定插入點位置以送出文本。

Alt + a 或 Ctrl + 數字鍵盤 `+` ：若有指定插入點位置，則會根據所記憶的位置送出其前的文本。若其前無所指定，則以目前插入點位置之前的文本送出。
> 今 Alt + a 已增益為可自動依每頁行/段數，判斷該送出去CTP的文本。在自動連續輸入模式（乃至快捷模式）時，常與 Ctrl + 數字鍵盤 `-` 配合運用 20250217
> 在自動連續輸入模式時按下F8、F9、F12等效 20250218
...
Ctrl + Alt + p ： 清除選取內的`<p>` 在行長不一而自動加上段落標記的功能失誤時很實用
...
>若無選取範圍時就執行 Alt + w 之功能 20250131大年初三 感恩感恩　讚歎讚歎　GitHub Copilot大菩薩　南無阿彌陀佛　讚美主
...
> 即插入點所在視為後面有夾注的正文（則放掉Shift鍵，只按Alt鍵）
...
> 可清除插入點之前的所有空格「　」、空白「􏿽」、「<p>」、「}}」及〖〗之間的全部內容
...
> 若選取的是`<p>`，則逕自輸入全形空格以取代之；在快捷模式連續輸入時，因為檢查行長的緣故，會自行選取`<p>`，並在剪貼簿中嵌入全形空格以供貼上備用。故亦可經由如此方法以便輸入。看個人習慣與順手。蓋多因挪抬的緣故留有空格，而源文本未予適當保留的緣故。感恩感恩　讚歎讚歎　南無阿彌陀佛　讚美主 20260102

Alt + [ ：將選取文字前後加上`〖` 與 `〗`。即如文字外加框或圈、印鑑或牌記外框效果。
> 在取代輸入模式時會自動向右/後多選取一個字，如顯示的游標寬度一樣（省了手動選取一個字）。
...
>還有其餘類似功能，詳後開列。
#### 自動標點
Alt + F10 、 Alt + F11 ： 將textBox1中選取的文字送去《古籍酷》自動標點。
...
> 若為2個漢字以上則改查組字（即部件形構組字檢索）

Alt + x ：檢索《康熙字典網上版》（x=xi，「熙」漢語拼音）

Alt + c ：檢索《漢語大詞典》（c=ci，「詞」漢語拼音）

Alt + d 或 Ctrl + Alt + j ：以選取文字進行[《看典古籍·古籍全文檢索》](https://kandianguji.com/search) (d=dian 典；j=籍 ji) 20241008
...
## Word VBA 常用實用功能鍵總匯
    請在 Word 文件中先選取欲查找的內容，再按以下諸鍵檢索；若無選取，則預設以插入點所在位置後第1個字送出檢索。（即檢索單字時，可以不選取，但插入點要放在要檢索的單字前面）
...
- Alt + s 或 Ctrl + Shift + Alt + e - ：**單字**檢索《[白雲深處人家·說文解字注](https://homeinmists.ilotus.org/shuowen/dyc.php)》（段注《說文》）之字頭。（s ：說 shuo 的 s，e:文 wen 的 e） 20250120
...
- Alt + m ： 以選取文字 search CTP的《史記三家注》 （m=ma(司馬遷的馬） 20241030  以選取文字 search史記三家注並於於選取處插入檢索結果之超連結 （s=shi 史；j=ji 記 ） 20241005
...
- Ctrl + Alt + j 或 Alt + d 以文件中選取文字進行[《看典古籍·古籍全文檢索》](https://kandianguji.com/search) （j=籍 ji；d=典 dian）
...
- Alt + l 大陸引號改臺灣引號（若無選取即以整份文件執行置換）（L，形狀像單引號`「`或`」`故）
  > 有選取只執行選取區，無則整份文件
...
>> 已解決以上問題！！可以在背影工作了!!!詳 JsClick 方法及其類別之說明 20251231 元旦除夕

>> 唯 **★★★★★重要！！！！！** 在程式自動化連續輸入時 *（或其實即使有其他人在編輯時當也適用）* ，盡量不要同時編輯同一單位（篇卷 chapter）的文本，在末學編輯[此頁](https://ctext.org/library.pl?if=gb&file=95825&page=272)時就被[此頁](https://ctext.org/library.pl?if=gb&file=95825&page=266)的手動編輯所蓋覆。蓋時間戳記同在「2025-12-31 03:13:05」同時衝突的緣故，伺服器反應不過來吧。詳 [History 頁面記錄](https://ctext.org/wiki.pl?if=gb&chapter=980722&action=history)可徵。蓋即便前後頁不相連，也沒OCR_MATCH的標籤，也會如此！！

>啟用快速模式時會順便啟用連續輸入模式。

>按下CapsLock按鍵，使Caps Lock燈亮 來剎車。 20260208
...
Alt + Shift + r ：在Selenium模式+手動輸入模式下、關閉所在Chrome瀏覽器右側之分頁。（因應《古籍酷》連線不暢所衍生之措施）20231026
...
> Ctrl + - ： 若插入點在textBox1內容的末端，且其前為分行/分段符號，則將「Next page:」方塊內容的第一行/段，追加為textBox1內容的尾綴，並清除「Next page:」方塊內容此行/段文字
...
> 在連續輸入翻頁快完成時按下Ctrl ，以略去對話方塊之顯示

>> 按 Ctrl 或左邊的Shift 切換快捷模式（會有音效）20250206蛇年初九

>> 按 Caps 關閉快捷模式（會有音效）

>>>在對話方塊按下確定時同時按下Ctrl鍵，也可以啟動快捷模式 20250215

>>>>在快捷模式啟用時按下左邊的 Shift 可有效煞停斷開，停止快捷模式（因為多工時易遭誤按，故改用Caps鍵就好)
...
Insert : 如 MS Word ，切換插入/取代文字模式（hit! 還原機制亦大致成功了。還原上限為300次。Ctrl + z ： 支援打字輸入時及取代文字時的還原。）
...
Alt + r ：將夾注文本錯亂倒置者重整，如「{{雪電}}　{{雨霧}}　{{霽虹}}　{{雷}}」這樣的文本，改成「{{雪􏿽雨􏿽霽􏿽雷、電􏿽霧􏿽虹　　}}」
...
Shift + Enter 或 Alt + Enter（ 與 Ctrl + 滑鼠左鍵 同） ： 新行、折行、分行，並可依插入點所在行之縮排情形，自動補上前綴全形空格 20250318
...
Alt + Shift + w ： 將執行 MergeParagraphsAtCaretWithShift 方法，插入點所在行將視為夾注的第1行，並將其後的1行/段合併上來，前後分別加上「{{」和「}}」，然後再將它們後面的那1行也合併上來。最後，插入點將停留在最後合併上來的那行/段文字的起始處。
...
Ctrl + y ： 重做（即復原還原的動作），目前上限為300個記錄

Ctrl + z ： 還原文本，目前上限為300個記錄（原為50）
...
拖曳圖文對照頁面的`[編輯]（Edit）`元件（的網址）在 textBox1 上放開，則會啟動WordVBA巨集的`中國哲學書電子化計劃.清除頁前的分段符號`程序 以整理文本 20250218
...
- [查字.mdb](https://www.dropbox.com/scl/fi/vze6vqx4ooj7dwd6met53/.mdb?rlkey=mh1kl48l75di505wedo771naw&dl=0)：此為資料檔；資料結構可參看此檔
